...
int CMainWindow::OnCreate(LPCREATESTRUCT lpcs)
{
	...
	m_wndLBTile.Create(_T("Typefaces"), WS_CHILD|WS_VISIBLE|SS_LEFT,
		rect, this);

	rect.SetRect(m_cxChar * 2, m_cyChar * 2, m_cxChar * 48, m_cyChar * 18);

	m_wndListBox.CreateEx(WS_EX_CLIENTEDGE, _T("listbox"), NULL, WS_CHILD|WS_VISIBLE|LBS_STANDARD,
		rect, this, IDC_LISTBOX);

	rect.SetRect(m_cxChar * 2, m_cyChar * 19, m_cxChar * 48, m_cyChar * 20);

	m_wndCheckBox.Create(_T("Show TrueType fonts only"), WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,
		rect, this, IDC_CHECKBOX);
	...
	m_wndSampleText.Create(_T(""), WS_CHILD|WS_VISIBLE|SS_CENTER,
		rect, this, IDC_SAMPLE);

	rect.SetRect(m_cxChar * 50, m_cyChar * 2, m_cxChar * 66, m_cyChar * 4);

	m_wndPushButton.Create(_T("Print Sample"), WS_CHILD|WS_VISIBLE|WS_DISABLED|BS_PUSHBUTTON, rect, this, IDC_PRINT);

	m_wndLBTile.SetFont(&m_fontMain, FALSE);
...
void CMainWindow::OnSelChange()
{
	...
	else
	{
		...
		m_fontSample.CreateFont(-m_cyChar * 2, 0, 0, 0, FW_NORMAL,
			...
			CLIP_CHARACTER_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH|FF_DONTCARE, strFaceName);
